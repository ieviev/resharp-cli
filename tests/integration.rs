...
#[test]
fn secrets_aws_key() {
    let (out, _) = run_stdin(&["--secrets"], "normal line\nAKIAIOSFODNN7EXAMPLE\n");
    assert!(out.contains("AKIAIOSFODNN7EXAMPLE"));
    assert!(!out.contains("normal line"));
}

#[test]
fn secrets_github_pat() {
    let (out, _) = run_stdin(&["--secrets"], "normal line\nghp_ABCDEFghijklmnop1234567890abcdefghij\n");
    assert!(out.contains("ghp_"));
}

#[test]
fn secrets_private_key() {
    let (out, _) = run_stdin(&["--secrets"], "normal line\n-----BEGIN RSA PRIVATE KEY-----\n");
    assert!(out.contains("PRIVATE KEY"));
}

#[test]
fn secrets_key_value() {
    let (out, _) = run_stdin(&["--secrets"], "normal line\nAPI_KEY=sk-proj-abc123XYZ789longTokenHere456\n");
    assert!(out.contains("API_KEY"));
}

#[test]
fn secrets_skips_normal() {
    let (out, _) = run_stdin(&["--secrets"], "just a regular comment\nnormal line\n");
    assert_eq!(out, "");
}
